...
class FlipkartApi
  ...
  #
  def initialize(fk_userid, fk_token, version='v1.1.0')
  ...
  def get_category_products_api(category_name)
    ...
    p JSON.parse(get_categories("json"))["apiGroups"]["affiliate"]["apiListings"][category_name]["availableVariants"][@version]["get"]
  ...
  def get_products_by_category(category_name)
    ...
    p get_category_products_api(category_name)
  ...
  def search(key, format,  max_result=10)
    ...
    rest_url = "#{@api}/1.0/search.#{format}?query=#{key}&resultCount=#{max_result}"
